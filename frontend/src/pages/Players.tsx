...
export const Players: React.FC = () => {
  ...
  const sortedPlayers = [...filteredPlayers].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'winPercentage':
        return getWinPercentage(b.wins, b.losses) - getWinPercentage(a.wins, a.losses);
      case 'gamesPlayed':
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              <div className="relative">
                <img
                  src={player.profileImage || `https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=150`}
                  alt={player.name}
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <Badge variant={getRatingBadgeVariant(player.rating)} size="sm">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-gray-900">{winPercentage}%</div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">View player rankings and statistics</p>
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-purple-600">
              {Math.round(players.reduce((acc, p) => acc + getWinPercentage(p.wins, p.losses), 0) / players.length)}%
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {sortedPlayers.slice(0, 3).map((player, index) => (
              <div key={player.id} className="text-center p-4 bg-gray-50 rounded-lg">
                <div className={`text-3xl mb-2 ${index === 0 ? 'text-yellow-500' : index === 1 ? 'text-gray-400' : 'text-orange-600'}`}>
                  {index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉'}
                </div>
                <img
                  src={player.profileImage || `https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=150`}
                ...
                <div className="text-sm text-gray-600">Rating: {player.rating.toFixed(1)}</div>
                <div className="text-sm text-gray-600">{getWinPercentage(player.wins, player.losses)}% Win Rate</div>
              </div>
            ))}
          </div>
        </CardContent>
        ...
        {selectedPlayer && (
          <div className="space-y-6">
            <div className="flex items-center space-x-6">
              <img
                src={selectedPlayer.profileImage || `https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=150`}
              ...
              <div>
                ...
                <div className="flex items-center space-x-3 mt-2">
                  <Badge variant={getRatingBadgeVariant(selectedPlayer.rating)} size="md">
                    <Star className="w-4 h-4 mr-1" />
                    {selectedPlayer.rating.toFixed(1)} Rating
                  </Badge>
                  <Badge variant="info" size="md">
                    Rank #{sortedPlayers.findIndex(p => p.id === selectedPlayer.id) + 1}
                  </Badge>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold text-gray-900">{selectedPlayer.wins}</div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold text-gray-900">{selectedPlayer.losses}</div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  ...
                  <div className="text-2xl font-bold text-gray-900">
                    {getWinPercentage(selectedPlayer.wins, selectedPlayer.losses)}%
                  </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{selectedPlayer.gamesPlayed}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Current Rating</span>
                      <span className={`font-medium ${getRatingColor(selectedPlayer.rating)}`}>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{(selectedPlayer.rating + 0.2).toFixed(1)}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <span className="text-sm text-gray-600">Won vs Mike Johnson</span>
                      <span className="text-xs text-gray-400">2 days ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Won vs Emma Davis</span>
                      <span className="text-xs text-gray-400">5 days ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Lost to Sarah Wilson</span>
                      <span className="text-xs text-gray-400">1 week ago</span>
                    </div>
                    <div className="flex items-center space-x-3">
                      <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                      <span className="text-sm text-gray-600">Won vs David Brown</span>
                      <span className="text-xs text-gray-400">1 week ago</span>
                    </div>
                  </div>
                </CardContent>
